...
public class LightTrackerISTest {
    ...
    private static final String lightingDeviceName = "OvenAppliance";
    ...
    public void before() {
        ...
        MongoDatabase datastore = mongoClient.getDatabase("3psslighttrackertest")
    ...
    private void subscriptionTest() throws Exception {
        ...
        Response response = client
                .target(String.format("http://localhost:" + RULE.getLocalPort() + "/api/v1/homes/" + homeId.toString()
                        + "/subscription"))
                .request()
    ...
    @Test
    public void queryAnomalies() throws Exception {
        ...
        ama.setAnomalyState(LightTrackerAnomaly.STATE_LIGHTONANOMALY);

        lightTrackerAnomalyDao.createLightTrackerAnomaly(ama);

        // request the active anomolies.
        Response response = client
                .target(String.format("http://localhost:" + RULE.getLocalPort() + "/api/v1/homes/" + homeId.toString()
                        + "/passthrough/anomalies"))
                .request().get();

        assertThat(response.getStatus()).isEqualTo(200);

        List<LightTrackerReportedAnomaly> anomalies = response.readEntity(
                new GenericType<List<LightTrackerReportedAnomaly>>() {
                });

        assertThat(anomalies.size()).isEqualTo(1);
        assertThat(anomalies.get(0).getDeviceId()).isEqualTo(lightingDeviceId);
        assertThat(anomalies.get(0).getDeviceName()).isEqualTo(lightingDeviceName);

        response.close();

        // send an ignore command.
        response = client
                .target(String.format("http://localhost:" + RULE.getLocalPort() + "/api/v1/homes/" + homeId.toString()
                        + "/passthrough/anomalies/" + lightingDeviceId + "/ignore"))
